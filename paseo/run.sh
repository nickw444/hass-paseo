...
command -v cursor-agent >/dev/null 2>&1 || fatal "Cursor Agent CLI is missing from the runtime image."
cursor_agent_version="$(cursor-agent --version 2>/dev/null || true)"
[[ -n "${cursor_agent_version}" ]] || fatal "Cursor Agent CLI is not executable."
command -v copilot >/dev/null 2>&1 || fatal "GitHub Copilot CLI is missing from the runtime image."
copilot_version="$(copilot --version 2>&1)" || fatal "GitHub Copilot CLI is not executable: ${copilot_version:-unknown}"
[[ -n "${copilot_version}" ]] || fatal "GitHub Copilot CLI returned no version output."
